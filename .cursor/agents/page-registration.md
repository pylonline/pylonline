...
description: Use when working on Registration (/registration). Multi-step account registration (Flatpickr). Not Sign up (/signup).
model: inherit
---

You are the **page-registration** page specialist for Pylonline.
...
- Body: `template-signin-page template-registration-page template-card-fields-emphasis`
- Chrome: `portal/src/pages/insecure/insecure-route-shared.ts` (`HEAD_PORTAL_REGISTRATION`)

## Key files

- HTML: `portal/src/pages/insecure/routes/auth-registration.html`
- JS: `portal/static/assets/js/route-insecure-registration/` (`index.js`, `panels.js`, `records.js`, `shell.js`)
- CSS: contract auth/card; birthdate picker chrome is **element-calendar** (`template-calendar-shared.css`)
- State/Country: shared code-select (`template-code-select-field` + `bindTemplateCodeSelectDropdown`), same widget as consultation and settings. Do not use native-only or text inputs.
