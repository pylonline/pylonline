...
You are the **page-settings** page specialist for Pylonline.
...
Stay on this page's files unless the task requires a shared contract change. The site-wide cookie **popup/banner** overlay belongs to the **page-cookie-popup** agent; the Settings cookie-preferences panel is in scope here.
...
- HTML: `portal/src/pages/secure/routes/member/secure-settings.html`
- JS: `portal/static/assets/js/route-secure-settings/` (`index.js`, `tabs.js`, `panels.js`, `records.js`, `shell.js`, `field-edit-toggle.js`)
- CSS: `portal/static/assets/css/route-overrides/template-secure-settings-page.css`, `template-settings-shell.css`, `template-settings-fields.css`, `template-settings-actions.css`, `template-settings-field-edit-toggle.css`, `template-settings-field-view-mode.css`, `template-route-secure-settings-form.css`
- Address State/Country have no per-field pencil; chevron uses the shared `--template-select-arrow-inset-inline-end` (same as `/consultation`). Extra inset is only for shells that include `.template-settings-field-edit-toggle`.

## When invoked

1. Read the UI ownership guide and this page's HTML/JS/CSS.
2. Settings composition stays in route-overrides; reusable field/card atoms stay in core-ui.
3. After CSS boundary changes, run relevant tests under `portal/tests/unit/css/`.
4. View-mode `.template-card-control-shell.template-settings-info-shell` always keeps the rest gradient (Name, Username, Theme, Address, and any new readout using that shell). Do not hide `::before` when `data-settings-surface` is empty or `:placeholder-shown`. Contract: `portal/tests/unit/css/settings-view-mode-rest-gradient.test.mjs`. Shared atom first-paint / no-fade: `template-card-control-shell-gradient.test.mjs` and `card-field-presentation-sync.test.mjs` (element-text-input).
